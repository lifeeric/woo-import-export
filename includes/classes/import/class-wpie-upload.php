...
class WPIE_Upload {
        ...
        protected function wpie_manage_import_file( $fileName = "", $fileDir = "", $wpie_import_id = 0 ) {
                ...
                if ( ! is_file( $current_file ) ) {
                        ...
                        return new \WP_Error( 'wpie_import_error', __( 'Uploaded file is empty', 'woo-import-export' ) );
                } elseif ( ! preg_match( '%\W(xml|zip|csv|xls|xlsx|xml|ods|txt|json|gz|tar)$%i', trim( $fileName ) ) ) {
                        unset( $filePath, $current_file, $file_data, $fileList, $active_file );

                        return new \WP_Error( 'wpie_import_error', __( 'Uploaded file must be XML, CSV, ZIP, XLS, XLSX, ODS, TXT, JSON, GZ, TAR', 'woo-import-export' ) );
